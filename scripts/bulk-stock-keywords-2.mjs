 ...
 * 第2弾: increaseKeyword: false（直接サジェスト）
 * ホームジム特化キーワードには inc:false の方が精度が高い
 *
 * Usage:
 *   node scripts/bulk-stock-keywords-2.mjs           # 本番実行
 *   node scripts/bulk-stock-keywords-2.mjs --dry-run
 */
...
const REQUIRED_TERMS = [
  ...
  '自宅 ジム', '家庭用 ジム',
  'ダンベルプレス', 'ダンベルカール', 'ダンベル メニュー',
];

// ── シード（第1弾と全く同じ85本、increaseKeyword だけ false に変更）─────
const SEEDS = [
  // guide
...
function scoreKeyword(item) {
  ...
  if (difficulty >= 60) return -100;
  if (volume < 30) return -20;
  const longTailBonus = text.length >= 8 ? 12 : 0;
  const intentBonus = /おすすめ|選び方|比較|初心者|作り方|費用|予算|設置|防音|後悔|必要|何畳|賃貸/.test(text) ? 10 : 0;
  return Math.round(volume / 100 + (60 - difficulty) * 1.5 + (50 - competition) * 0.8 + longTailBonus + intentBonus);
}
...
function inferCategory(keyword) {
  ...
  if (/ジムマット|ジョイントマット|トレーニングマット|床材|防音|防振|ゴムマット/.test(keyword)) return 'floor';
...
if (!apiKey) { console.error('❌ RAKKO_KEYWORD_API_KEY が設定されていません'); process.exit(1); }

let existingStock = [];
try { existingStock = JSON.parse(await fs.readFile(STOCK_PATH, 'utf8')); } catch { /* 初回 */ }
const existingKeywords = new Set(existingStock.map((item) => normalize(item.keyword)));

console.log('\n═══════════════════════════════════════════════════');
console.log(' 🏋️  ホームジム キーワードストック 第2弾');
console.log(' ★ increaseKeyword: false — 直接サジェスト');
console.log('═══════════════════════════════════════════════════');
console.log(`シード数: ${SEEDS.length}件 / 既存ストック: ${existingStock.length}件`);
...
if (!DRY_RUN) await fs.writeFile(STOCK_PATH, JSON.stringify(merged, null, 2) + '\n', 'utf8');

console.log('\n═══════════════════════════════════════════════════');
console.log(` 完了 | 取得: ${totalFetched}件 → 新規: ${totalNew}件 | エラー: ${errors}件`);
console.log(` ストック合計: ${merged.length} 件`);
if (!DRY_RUN) console.log(` 保存先: ${STOCK_PATH}`);
console.log('═══════════════════════════════════════════════════\n');

function normalize(k) { return k.replace(/\s+/g, ' ').trim().toLowerCase(); }
