...
export function buildAffiliatePromptSection(products: AffiliateProduct[]) {
  ...
  return `
アソシエイト商品カード方針:
- 商品カードは1記事につき最大3つまで挿入できる。
- 本文で個別商品の詳細、価格、サイズ、重量、レビュー、設置条件などに触れる場合は、その商品に対応する {{affiliate:商品ID}} を必ず直後または同じ小見出し内の自然な位置に入れる。
- 複数の商品を比較・紹介する記事では、本文で具体的に説明した商品ごとに商品カードを入れる。1つだけに絞らない。
- 関係が薄い記事では無理に入れない。ただし記事内容に自然に合う商品がある場合は、1〜3つの商品カードを入れる。
